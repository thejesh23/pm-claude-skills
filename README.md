...
**🆕 Latest — v35.0.0 (400 skills & everywhere you work):** the library crosses **400 skills across 57 bundles**, with a new **📣 `pm-pmm` Product Marketing bundle** — `win-loss-analysis`, `sales-enablement-kit`, `pricing-page-copy`, `analyst-relations-brief`, `customer-advisory-board`, `voice-of-customer-program`, `launch-tiering-framework`, `sales-demo-script`. Plus four new ways to reach every skill: a **Custom GPT** (live Actions API) and **Gemini Gem**, a **Raycast** extension and **Alfred** workflow, and a new CLI **`search`** command. The playground gains **bring-your-data** (pull a public GitHub issue/PR URL into your context), and `board-minutes` lands in `pm-business` (contributed by [@roian6](https://github.com/roian6)). *Earlier (v34.0):* the `pm-craft` bundle, the `brief-builder` grill skill & marketing depth. → **[Full release history & notes](https://github.com/mohitagw15856/pm-claude-skills/releases)**.
